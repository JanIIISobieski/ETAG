...
void PressureBuffer::write(volatile uint8_t* source, size_t length) {
...
void PressureBuffer::increment_and_check_push() {
...
#ifdef ETAG_DEBUG
    logger.update_timing_data(pressure_buffer_push);
#endif
    }
}

void PressureBuffer::reset_buffer(volatile BufferHelper<uint8_t>* buff) {
    *(buff->count_ptr) = 0;
    *(buff->time_ptr) = 0;
