...
void IMUBuffer::write(volatile int16_t* source, size_t length) {
...
void IMUBuffer::increment_and_check_push() {
...
#ifdef ETAG_DEBUG
    logger.update_timing_data(imu_buffer_push);
#endif
    }
}

void IMUBuffer::reset_buffer(volatile BufferHelper<int16_t>* buff) {
    *(buff->count_ptr) = 0;
    *(buff->time_ptr) = 0;
