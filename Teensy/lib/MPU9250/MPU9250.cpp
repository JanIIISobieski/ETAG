...
void MPU9250::calibrateMPU9250(float * gyroBias, float * accelBias)
{
  ...
  logger.print_array("Gyro Bias", gyroBias, 3);
  logger.print_array("Accel Bias", accelBias, 3);
}
...
void MPU9250::MPU9250SelfTest(float * destination)
{
  ...
  logger.print_array("Self Test", destination, 6);
}
...
uint8_t MPU9250::writeByteSPI(uint8_t registerAddress, uint8_t writeData)
{
  ...
  deselect();
  _spi->endTransaction();
// #ifdef SERIAL_DEBUG  //All SERIAL_DEBUG are from the library code, keeping in case adapting this code into the logger will be useful
...
bool MPU9250::sample() {
  if (!allow_reads) return false;  // if we are not allowing reads, then return false
  else {
    if (readByte(_I2Caddr, INT_STATUS)) {  // if this is not 0, then read the data
      readAccelData(&buffer[0]);
      readGyroData(&buffer[3]);
      readMagData(&buffer[6]);
      return true;  // return true after data read
    }
    else return false;  // no data read, return false
  }
}

bool MPU9250::init() {
    logger.print_message("In init of MPU9250");
    MPU9250SelfTest(this->selfTest);
    calibrateMPU9250(this->gyroBias, this->accelBias);
    initMPU9250();
    initAK8963(this->factoryMagCalibration);
